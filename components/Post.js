  ...
  return (
    <div className='bg-white my-7 border rounded-sm'>
        ...
        <p className='p-5 truncate'>
            {likes.length > 0 && (
                <p className='font-bold mb-1'>{likes.length} aprecieri</p>
            )}
        ...
        {session && (
            <form className='flex items-center p-4'>
                ...
                <input type="text" value={comment} onChange={e => setComment(e.target.value)} placeholder='Adaugati un comentariu...' className='border-none flex-1 focus:ring-0 outline-none' />
                <button type='submit' disabled={!comment.trim()} onClick={sendComment} className='font-semibold text-blue-400'>Posteaza</button>
            </form>
        )}
